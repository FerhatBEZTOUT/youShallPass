...
public class SemestreunActivity extends AppCompatActivity {
    private MyDatabaseHelpter myDB;
    private ArrayList<Module> moduleList;
    private String nom;
    private final int SEMESTRE = 1;
    ModuleAdapter moduleAdapter;
    RecyclerView recyclerView;
    TextView titre;
    Button btnAddModule, btnCalcMoy;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_semestreun);
        recyclerView = findViewById(R.id.recyclerViewS1);
        btnAddModule = findViewById(R.id.btnAddModule);
        btnCalcMoy = findViewById(R.id.btnCalcMoyS1);

        Intent i = getIntent();
        this.nom = i.getStringExtra("profil");
        titre = findViewById(R.id.titreS1);

        myDB = new MyDatabaseHelpter(SemestreunActivity.this);
        moduleList = new ArrayList<>();
        storedModules(nom,moduleList);
        remplirTitre(moduleList);
        moduleAdapter = new ModuleAdapter(SemestreunActivity.this, moduleList, nom);
        recyclerView.setAdapter(moduleAdapter);
        recyclerView.setLayoutManager(new LinearLayoutManager(SemestreunActivity.this));
        btnAddModule.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(SemestreunActivity.this, AddmoduleActivity.class);
    ...
    // Méthode pour calculer la moyenne d'un semestre
    ...
    private void remplirTitre(ArrayList<Module> moduleList) {
        if (moduleList.size()==0) {
            ...
            if (c.getCount()==0) {
                ...
                Log.d("position","S1Activity ligne 265");
